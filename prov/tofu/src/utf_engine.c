	       ...
	       struct utf_recv_cntr *ursp, struct utf_msgbdy *msgp, int sidx)
{
	...
	if ((idx = utfgen_explst_match(pkt)) != -1) {
	    ...
	    DEBUG(DLEVEL_ADHOC|DLEVEL_CHAIN) utf_printf("%s: unexpt msg. req(%p) idx(%d) tag(0x%lx) src(%ld) size(%ld) %s\n", __func__, req, idx, pkt->hdr.tag, pkt->hdr.src, pkt->hdr.size, msgp->ptype == PKT_RENDZ ? "rget_" : "put_");
	    switch(msgp->ptype) {
...
progress:
	...
	case EVT_LCL_REQ: /* a result of remote_add (armw8)  in TRANSMODE_AGGR
		...
		DEBUG(DLEVEL_CHAIN) utf_printf("%s: S_REQ_ROOM result = %s of swap\n", __func__, chain_addr_string(rslt));
	...
	if (recvstadd == 0) {
	    if (usp->smode == TRANSMODE_CHND && evt != EVT_CONT) {
		utf_printf("%s: dst(%d) recvoff(%d) should never happen. going to sleep\n", __func__, usp->dst, usp->recvoff);
		abort();
	    }
	    if (usp->smode == TRANSMODE_CHND) utf_printf("%s: going to sleep becasue this is EVT_CONT in chained\n", __func__);
	    DEBUG(DLEVEL_PROTOCOL) {
	...
	break;
    case S_REQ_RDV: /* smode is TRANMODE_CHAIN, here as a result of remote_put */
	if (evt != EVT_LCL) {
	    utf_printf("%s: S_REQ_RDV evt(%s) != EVT_LCL\n", __func__,  evnt_symbol[evt]);
	    abort();
	}
	if (IS_MSGBUF_FULL(usp)) {
	    utf_printf("%s: YI!!!!!!!!!!!!!!!! RECEIVER'S BUFFER FULL\n", __func__);
	    usp->ostate = usp->state; /* S_REQ_RDV */
	    usp->state = S_WAIT_BUFREADY;
	    break;
	}
	usp->rgetwait |= 0x1;
